...
<template>
  <div class="rounded-2xl bg-card border border-border p-4 sm:p-5 lg:p-6 h-full">
    <!-- Title -->
    <Skeleton v-if="loading" class="h-5 sm:h-6 w-32 sm:w-36 mb-3 sm:mb-4" />
    <h2 v-else class="text-base sm:text-lg font-heading font-semibold mb-3 sm:mb-4">Recent Activity</h2>

    <!-- Skeleton Loading -->
    <div v-if="loading" class="space-y-3 sm:space-y-4">
      <div v-for="i in 4" :key="i" class="flex items-start gap-2 sm:gap-3">
        <Skeleton class="h-7 w-7 sm:h-8 sm:w-8 rounded-lg" />
        <div class="flex-1">
          <Skeleton class="h-3 sm:h-4 w-full mb-1" />
          <Skeleton class="h-2.5 sm:h-3 w-14 sm:w-16" />
        </div>
      </div>
    </div>

    <!-- Empty State -->
    <div v-else-if="!hasActivities" class="text-center py-6 sm:py-8">
      <div class="h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-muted flex items-center justify-center mx-auto mb-2 sm:mb-3">
        <Clock class="h-5 w-5 sm:h-6 sm:w-6 text-muted-foreground" />
      </div>
      <p class="text-xs sm:text-sm text-muted-foreground">No recent activity yet</p>
    ...
    <ul v-else class="space-y-3 sm:space-y-4">
      ...
      >
        <div class="h-7 w-7 sm:h-8 sm:w-8 rounded-lg bg-muted flex items-center justify-center shrink-0">
          <component :is="iconMap[activity.icon]" class="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground" />
        </div>
        <div class="flex-1 min-w-0">
          <p class="text-xs sm:text-sm">{{ activity.text }}</p>
